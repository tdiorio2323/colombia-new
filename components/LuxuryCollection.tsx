...
const StarRating: React.FC<{ rating: number }> = ({ rating }) => {
    return (
        <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
                <StarIcon key={i} className={`w-5 h-5 ${i < rating ? 'text-brand-gold' : 'text-stone-600'}`} />
            ))}
        </div>
    );
};
  ...
  return (
    <section className="py-20 px-4">
      <div className="container mx-auto">
        <SectionTitle title="Luxury Collection" subtitle="Exclusive treasures for distinguished collectors" />
